...
class  Cloths(models.Model):
    ...
    @property
    def reviews(self):
        qs=self.reviews_set.all()#custom method
        return qs
    @property
    def avg_rating(self):
        ratings=self.reviews_set.all().values_list('rating',flat=True)#it is used to get a rating in reviews
        return sum(ratings)/len(ratings) if ratings else 0#it is used to find a avg rating
